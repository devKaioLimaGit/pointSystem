...
router.post("/reset-password", async (req, res) => {
  ...
  // Função para verificar se o identificador é um CPF válido (formatado ou não)
  function isCPF(identifier) {
    const cpfPattern = /^\d{3}\.\d{3}\.\d{3}-\d{2}$/; // Padrão para CPF formatado
    const numericPattern = /^\d{11}$/; // Padrão para CPF não formatado (somente números)
    return cpfPattern.test(identifier) || numericPattern.test(identifier);
  }

  // Verifica se o identificador é um e-mail, username ou CPF
  if (identifier.toString().indexOf("@") > -1) {
    // Caso seja e-mail, adiciona a condição para o e-mail
    conditions.push({ email: identifier });
  } else if (identifier.toString().indexOf(".") > -1 && !isCPF(identifier)) {
    // Caso seja username, mantemos o identifier como está, e diferenciamos CPF de username
    identifier = identifier.toUpperCase();
    const [name, surname] = identifier.split(".");
    console.log(name, surname);

    // Adiciona o primeiro objeto com a condição para o nome
    conditions.push({ name: name });

    // Adiciona o segundo objeto com a condição para o sobrenome
    conditions.push({ surname: { [Op.like]: `%${surname}%` } });
  } else if (isCPF(identifier)) {
    // Caso seja CPF, seja formatado ou não, formatamos o CPF
    console.log("É um CPF");
    identifier = formatCPF(identifier);
    conditions.push({ cpf: identifier });
  } else {
  ...
  try {
    ...
    let user = await Users.findOne({
      where: {
        [Op.or]: conditions, // Aqui passamos as condições diretamente
      },
    });

    if (!user) {
      // Se o usuário não for encontrado pelo CPF ou e-mail, tentar encontrar pelo nome
      const cpfOrEmailOrLogin = identifier
        .toLowerCase()
        .replace("@gmail.com", "");
